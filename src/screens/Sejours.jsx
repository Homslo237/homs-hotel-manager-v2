...
export default function Sejours({ onNouveauSejour }) {
  ...
  const handleProlonger = (id, ajout, supplement) => {
    setSejours(prev=>prev.map(s=>{
      ...
      if (s.type==='nuit') {
        const [j,m,a]=s.dateDepart.split('/').map(Number)
        const d=new Date(a,m-1,j); d.setDate(d.getDate()+ajout)
        const newDepart=d.toLocaleDateString('fr-FR')
        const nn=diffNuits(s.dateArrivee,newDepart)
        return {...s, dateDepart:newDepart, duree:`${nn} nuit${nn>1?'s':''}`, montant:nouveauMontant.toLocaleString('fr-FR')}
      } else {
        const [h,min]=s.heureDepart.split(':').map(Number)
        const total=h*60+min+ajout*60
        const newH=`${String(Math.floor(total/60)%24).padStart(2,'0')}:${String(total%60).padStart(2,'0')}`
        const nh=diffHeures(s.heureArrivee,newH)
        return {...s, heureDepart:newH, duree:`${nh} heure${nh>1?'s':''}`, montant:nouveauMontant.toLocaleString('fr-FR')}
      }
    }))
  }

  // Checkout : calcule supplément si dépassement > tolérance
  const handleCheckout = (s) => {
    ...
    setSejours(prev=>prev.map(x=>x.id===s.id?{...x,statut:'termine'}:x))
